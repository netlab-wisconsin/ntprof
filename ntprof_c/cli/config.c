...
void parse_io_sizes(const char *io_size_str, int *io_size_array, int *io_size_count) {
    char *token;
...
int parse_config(const char *filename, struct ntprof_config *config) {
    ...
    const char *io_type = iniparser_getstring(ini, "Workload:IO_TYPE", DEFAULT_IO_TYPE);
