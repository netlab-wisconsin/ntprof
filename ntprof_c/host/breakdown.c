...
void break_latency_read(struct profile_record* record,
  ...
  breakdown->nstack_submission = nvmet_tcp_done_recv_pdu -
                                 nvmet_tcp_try_recv_pdu;
  breakdown->nvmet_tcp_submission = nvmet_tcp_exec_read -
                                    nvmet_tcp_done_recv_pdu;
  breakdown->target_subsystem = nvmet_tcp_queue_response -
                                nvmet_tcp_exec_read;
  breakdown->nvmet_tcp_completion = nvmet_tcp_send_response -
...
void break_latency_write(struct profile_record* record,
  ...
  if (record->metadata.contains_r2t) {
    ...
    breakdown->nstack_submission = nvmet_tcp_done_recv_pdu -
                                   nvmet_tcp_try_recv_pdu;
    breakdown->nvmet_tcp_submission = nvmet_tcp_try_send_r2t -
                                      nvmet_tcp_done_recv_pdu;

    struct ntprof_stat* stat2 = &record->stat2;
    nvmet_tcp_try_recv_pdu = stat2->ts[0];
    u64 nvmet_tcp_handle_h2c_data_pdu = stat2->ts[1];
    u64 nvmet_tcp_exec_write = stat2->ts[2];
    u64 nmet_tcp_queue_response = stat2->ts[3];
    u64 nvmet_tcp_send_response = stat2->ts[4];

    breakdown->nstack_submission += nvmet_tcp_handle_h2c_data_pdu -
        nvmet_tcp_try_recv_pdu;
    breakdown->nvmet_tcp_submission += nvmet_tcp_exec_write -
        nvmet_tcp_handle_h2c_data_pdu;
    breakdown->target_subsystem = nmet_tcp_queue_response -
                                  nvmet_tcp_exec_write;
    breakdown->nvmet_tcp_completion = (nvmet_tcp_send_response -
                                       nmet_tcp_queue_response) * 2;
  } else {
    ...
    int target_end_to_end = nvmet_tcp_send_response - nvmet_tcp_try_recv_pdu;
