...
static void categorize_records_of_each_core(struct per_core_statistics *stat) {
    ...
    list_for_each_entry_safe(rec, tmp, &stat->incomplete_records, list) {
        pr_cont("remove [req->tag=%d, req->cmdid=%d], ", rec->metadata.req_tag, rec->metadata.cmdid);
        list_del(&rec->list);
        kfree(rec);
    }
    pr_info("");
...
void summarize_category(struct categorized_records *cat, struct category_summary *cs) {
