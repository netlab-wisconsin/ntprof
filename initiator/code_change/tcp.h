...
static inline void tcp_snd_cwnd_set(struct tcp_sock *tp, u32 val, char* caller, int acked)
{
	WARN_ON_ONCE((int)val <= 0);
	if(ntohs(tp->inet_conn.icsk_inet.inet_dport) == 4420) {
		pr_info("[TCPLOG-CWND] TID: %d, TS: %llu, sport: %d, cwnd: %u, old_cwnd: %d, acked: %d, pktout: %u, snd_nxt: %u, snd_una: %u, snd_wnd: %u, snd_ssthresh: %u, caller: %s\n", 
				current->pid, ktime_get_ns(), ntohs(tp->inet_conn.icsk_inet.inet_sport), val, tp->snd_cwnd, acked, tcp_packets_in_flight(tp), tp->snd_nxt, tp->snd_una, tp->snd_wnd, tp->snd_ssthresh, caller);
	}
	tp->snd_cwnd = val;
}
...
static inline void tcp_slow_start_after_idle_check(struct sock *sk)
{
	...
	if (delta > inet_csk(sk)->icsk_rto) {	
		u64 ts = ktime_get_ns();
		if(ntohs(tp->inet_conn.icsk_inet.inet_dport) == 4420) {
			pr_info("[TCPLOG] TID: %d, TS: %llu, sport: %d, tcp_slow_start_after_idle_check calls tcp_cwnd_restart. delta: %d, rto: %d, lsndtime: %d, HZ: %d\n", 
				current->pid, ts, ntohs(tp->inet_conn.icsk_inet.inet_sport), delta, inet_csk(sk)->icsk_rto, tp->lsndtime, HZ);
		}
		tcp_cwnd_restart(sk, delta, ts);
	}
}
