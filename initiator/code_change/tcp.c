...
void tcp_init_sock(struct sock *sk)
{
	 ...
	 */
	tcp_snd_cwnd_set(tp, TCP_INIT_CWND, "tcp_init_sock", "-1");
...
void tcp_skb_entail(struct sock *sk, struct sk_buff *skb)
{
	...
	if(ntohs(tp->inet_conn.icsk_inet.inet_dport) == 4420) {
		pr_info("[TCPLOG] TID: %d, TS: %llu, sport: %d, tcp_skb_entail calls tcp_slow_start_after_idle_check. \n", 
...
#endif
		if(ntohs(tp->inet_conn.icsk_inet.inet_dport) == 4420) {
			pr_info("[TCPLOG] TID: %d, TS: %llu, sport: %d, tcp_build_frag calls tcp_skb_entail. \n", 
				current->pid, ktime_get_ns(), ntohs(tp->inet_conn.icsk_inet.inet_sport));
		}
...
new_segment:
			...
			if(ntohs(tp->inet_conn.icsk_inet.inet_dport) == 4420) {
				pr_info("[TCPLOG] TID: %d, TS: %llu, sport: %d, tcp_sendmsg_locked calls tcp_skb_entail. \n", 
					current->pid, ktime_get_ns(), ntohs(tp->inet_conn.icsk_inet.inet_sport));
			}
...
int tcp_disconnect(struct sock *sk, int flags)
{
	...
	icsk->icsk_delack_max = TCP_DELACK_MAX;
	tp->snd_ssthresh = TCP_INFINITE_SSTHRESH;
	tcp_snd_cwnd_set(tp, TCP_INIT_CWND, "tcp_disconnect", -1);
	tp->snd_cwnd_cnt = 0;
