...
static int nvme_tcp_init_connection(struct nvme_tcp_queue *queue)
{
	...
	if (le32_to_cpu(icresp->hdr.plen) != sizeof(*icresp)) {
		pr_err("queue %d: bad pdu length returned %d, le32_to_cpu=%d, sizeof(*icresp)=%d\n",
			nvme_tcp_queue_id(queue), icresp->hdr.plen, le32_to_cpu(icresp->hdr.plen), sizeof(*icresp));
		goto free_icresp;
	}
