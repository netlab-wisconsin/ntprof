...
#define LOCAL_PORT ntohs(tp->inet_conn.icsk_inet.inet_sport)
#define REMOTE_PORT ntohs(tp->inet_conn.icsk_inet.inet_dport)
...
void tcp_enter_loss(struct sock *sk)
{
	...
	int new_cwnd = tcp_packets_in_flight(tp) + 1;
	// if(LOCAL_PORT == 4420) {
		trace_cwnd_tcp_enter_loss(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
	// }
...
static void tcp_undo_cwnd_reduction(struct sock *sk, bool unmark_loss)
{
	...
	if (tp->prior_ssthresh) {
		...
		int new_cwnd = icsk->icsk_ca_ops->undo_cwnd(sk);
		// if(LOCAL_PORT == 4420) {
			trace_cwnd_tcp_undo_cwnd_reduction(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
		// }
...
void tcp_cwnd_reduction(struct sock *sk, int newly_acked_sacked, int newly_lost, int flag)
{
	...
	int new_cwnd = tcp_packets_in_flight(tp) + sndcnt;
	// if(LOCAL_PORT == 4420) {
		trace_cwnd_tcp_cwnd_reduction(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
	// }
...
static inline void tcp_end_cwnd_reduction(struct sock *sk)
{
	...
	if (tp->snd_ssthresh < TCP_INFINITE_SSTHRESH &&
		...
		int new_cwnd = tp->snd_ssthresh;
		// if(LOCAL_PORT == 4420) {
			trace_cwnd_tcp_end_cwnd_reduction(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
		// }
...
static void tcp_mtup_probe_success(struct sock *sk)
{
	...
	int new_cwnd = max_t(u32, 1U, val);
	// if(LOCAL_PORT == 4420) {
		trace_cwnd_tcp_mtup_probe_success(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
	// }
				  ...
				  int num_dupack, int *ack_flag, int *rexmit)
{
	...
	default:
		...
		if (icsk->icsk_ca_state < TCP_CA_CWR &&
			...
			int new_cwnd = tcp_snd_cwnd(tp) + 1;
			// if(LOCAL_PORT == 4420) {
				trace_cwnd_tcp_mtup_probe_failed(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
			// }
			tcp_snd_cwnd_set(tp, new_cwnd);
			       ...
			       struct tcp_sacktag_state *sack, bool ece_ack)
{
	...
	for (skb = skb_rb_first(&sk->tcp_rtx_queue); skb; skb = next) {
		...
		// if(LOCAL_PORT == 4420) {
			trace_pkt_tcp_clean_rtx_queue(tcp_packets_in_flight(tp), tcp_snd_cwnd(tp), LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
		// }
...
void tcp_init_transfer(struct sock *sk, int bpf_op, struct sk_buff *skb)
{
	...
	if (tp->total_retrans > 1 && tp->undo_marker){
		// if(LOCAL_PORT == 4420){
			trace_cwnd_tcp_init_transfer(tcp_snd_cwnd(tp), 1, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
		// }
		tcp_snd_cwnd_set(tp, 1);
	}
	else{
		int new_cwnd = tcp_init_cwnd(tp, __sk_dst_get(sk));
		// if(LOCAL_PORT == 4420){
			trace_cwnd_tcp_init_transfer(tcp_snd_cwnd(tp), new_cwnd, LOCAL_PORT, REMOTE_PORT, ktime_get_ns());
		// }
		tcp_snd_cwnd_set(tp, new_cwnd);
	}
	tp->snd_cwnd_stamp = tcp_jiffies32;
