...
for workload_type in "${workload_types[@]}"; do
    for io_depth in "${io_depths[@]}"; do
        ...
        fio_output=$(sudo fio --name=test --filename=/dev/nvme0n1 --rw=$workload_type --ioengine=libaio --numjobs=16 --time_based --runtime=20 --group_reporting --bs=$block_size --direct=1 --iodepth=$io_depth)
