...
recv:
	...
	// if(queue->nvme_sq.qid == 12)
	// pr_info("MYLOG: recv pdu, read %d bytes from socket, sid: %d, \n", len, queue->nvme_sq.qid);
...
static int nvmet_tcp_try_recv_data(struct nvmet_tcp_queue *queue)
{
	...
	while (msg_data_left(&cmd->recv_msg)) {
		...
		// if(queue->nvme_sq.qid == 12)
		// pr_info("MYLOG: recv data, read %d bytes from socket, sid: %d, \n", ret, queue->nvme_sq.qid);
		...
		struct socket *newsock)
{
	...
	queue->cnt = 0;
	atomic_long_set(&queue->read, 0);
	atomic_long_set(&queue->write, 0);

	/** print log */
	/** print the queue idx, sq id, cq id, sport, dport */
	pr_info("MYLOG: nvmet_tcp_alloc_queue is almost done, qidx: %d, sid: %d, cid: %d, sport: %d, dport: %d\n",
			queue->idx, queue->nvme_sq.qid, queue->nvme_cq.qid, ntohs(inet_sk(queue->sock->sk)->inet_sport), ntohs(inet_sk(queue->sock->sk)->inet_dport));
