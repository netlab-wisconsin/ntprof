...
void init_raw_tcp_stat(struct _tcp_stat *stat) {
  int i;
  for (i = 0; i < MAX_QID; i++) {
...
void on_pkt_tcp_event_new_data_sent(void *ignore, u32 packeg_in_flight,
  ...
  if (remote_port_filter(local_port) && port2qid(remote_port) != -1 && tcp_to_sample() ){
...
void copy_tcp_stat(struct tcp_stat *dst, struct _tcp_stat *src) {
  int i;
  for (i = 0; i < MAX_QID; i++) {
...
int init_tcp_proc_entries(void) {
  entry_tcp_dir = proc_mkdir("tcp", parent_dir);
  if (!entry_tcp_dir) {
    pr_err("Failed to create tcp\n");
    return -ENOMEM;
  }
